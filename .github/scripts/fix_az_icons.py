...
def fix_az_page(html):
    ...
    if m:
        ...
        # .float-wa svg ve .float-music svg'ye fill:#fff ekle (yoksa)
        def add_fill(m):
            block = m.group(0)
            if 'fill' not in block:
                return block.rstrip('}') + ' fill: #fff; }'
            return block
        content = re.sub(r'\.float-wa\s+svg\s*\{[^}]*\}', add_fill, content)
        content = re.sub(r'\.float-music\s+svg\s*\{[^}]*\}', add_fill, content)
    ...
    # 4. float-wa içindeki SVG'yi yeni SVG ile değiştir (fill="#fff" dahil)
    html = re.sub(
